<form action="{{ route('Submit Employee Quarantine Approval', ['id' => $id]) }}" method="POST" autocomplete="off">
    ...
    @if(Auth::user()->is_superuser() || Auth::user()->is_health_officer())
        ...
        <div class="form-group">
            ...
            <div class="custom-control custom-radio">
                <input type="radio" name="recommendation" value="Quarantine dates until cleared by residence MHO" {{ $quarantine->recommendation == "Quarantine dates until cleared by residence MHO" ? 'checked' : '' }} class="custom-control-input" id="mho" required>
                <label class="custom-control-label" for="mho">Quarantine dates until cleared by residence MHO</label>
            </div>
            <div class="custom-control custom-radio">
                <input type="radio" name="recommendation" value="Continue reporting to office" {{ $quarantine->recommendation == "Continue reporting to office" ? 'checked' : '' }} class="custom-control-input" id="office" required>
                <label class="custom-control-label" for="office">Continue reporting to office</label>
            </div>
            <div class="custom-control custom-radio">
                <input type="radio" name="recommendation" value="Report to local health authorities" {{ $quarantine->recommendation == "Report to local health authorities" ? 'checked' : '' }} class="custom-control-input" id="report" required>
                <label class="custom-control-label" for="report">Report to local health authorities</label>
            </div>
            <div class="custom-control custom-radio">
                <input type="radio" name="recommendation" value="To monitor for signs and symptoms using the health monitoring form" {{ $quarantine->recommendation == "To monitor for signs and symptoms using the health monitoring form" ? 'checked' : '' }} class="custom-control-input" id="monitor" required>
    ...
    <div class="form-group pt-2 mb-1">
        ...
        @if(Auth::user()->is_superuser() || Auth::user()->is_health_officer())
            <a href="{{ route('Remove Employee Quarantine', ['id' => $quarantine->id]) }}" class="btn btn-sm btn-success btn-block">Mark as Done</a>
        @endif
    </div>
